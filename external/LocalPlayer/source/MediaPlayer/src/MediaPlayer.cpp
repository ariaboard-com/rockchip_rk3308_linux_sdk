...
MediaPlayer::SourceId MediaPlayer::setSource(std::shared_ptr<rkpCommon::rkp::attachment::AttachmentReader> reader) {
...
void MediaPlayer::handleSetVolume(std::promise<bool>* promise, int8_t volume) {
    ...
    auto toGstVolume =
        Normalizer::create(RKP_SET_VOLUME_MIN, RKP_SET_VOLUME_MAX, GST_SET_VOLUME_MIN, GST_SET_VOLUME_MAX);
    if (!toGstVolume) {
...
void MediaPlayer::handleAdjustVolume(std::promise<bool>* promise, int8_t delta) {
    ...
    auto toGstDeltaVolume =
        Normalizer::create(RKP_ADJUST_VOLUME_MIN, RKP_ADJUST_VOLUME_MAX, GST_ADJUST_VOLUME_MIN, GST_ADJUST_VOLUME_MAX);
...
void MediaPlayer::handleGetSpeakerSettings(
    ...
    auto toRKPVolume =
        Normalizer::create(GST_SET_VOLUME_MIN, GST_SET_VOLUME_MAX, RKP_SET_VOLUME_MIN, RKP_SET_VOLUME_MAX);
    if (!toRKPVolume) {
    ...
    if (!toRKPVolume->normalize(gstVolume, &rkpVolume)) {
        ACSDK_ERROR(LX("handleGetSpeakerSettingsFailed").d("reason", "normalizeVolumeFailed"));
        promise->set_value(false);
        return;
    }

    // RKP Volume will be between 0 and 100.
    settings->volume = static_cast<int8_t>(std::round(rkpVolume));
    settings->mute = mute;
        ...
        case GST_MESSAGE_EOS:
            ...
            if (GST_MESSAGE_SRC(message) == GST_OBJECT_CAST(m_pipeline.pipeline)) {
                if (!m_source->handleEndOfStream()) {
                    localPlayerSDK::rkpCommon::utils::logger::LogEntry* errorDescription =
...
void MediaPlayer::handleSetUrlSource(
    ...
    m_urlConverter = localPlayerSDK::playlistParser::UrlContentToAttachmentConverter::create(
    ...
    std::shared_ptr<rkpCommon::rkp::attachment::AttachmentReader> reader =
        attachment->createReader(rkpCommon::rkp::attachment::AttachmentReader::Policy::BLOCKING);
    if (!reader) {
