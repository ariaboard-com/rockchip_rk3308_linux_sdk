/*
 * ExceptionEncounteredSenderTest.cpp
 *
...
#include <LPCommon/RKP/ExceptionEncounteredSender.h>
#include <LPCommon/SDKInterfaces/MockMessageSender.h>
#include <LPCommon/Utils/JSON/JSONUtils.h>
...
public:
     ...
     */
    void send(std::shared_ptr<rkp::ExceptionEncounteredSender> exceptionencounteredsender);
     ...
     */
    void verifyMessage(std::shared_ptr<rkpCommon::rkp::MessageRequest> request);
...
private:
    ...
    rkp::ExceptionErrorType m_error;
...
void ExceptionEncounteredEvent::send(std::shared_ptr<rkp::ExceptionEncounteredSender> exceptionEncounteredSender) {
    exceptionEncounteredSender->sendExceptionEncountered(m_unparsedDirective, m_error, m_errorDescription);
}

void ExceptionEncounteredEvent::verifyMessage(std::shared_ptr<rkpCommon::rkp::MessageRequest> request) {
...
protected:
    ...
    std::shared_ptr<rkpCommon::sdkInterfaces::test::MockMessageSender> m_mockMessageSender;

    /// The @c ExceptionEncounteredSender to test
    std::shared_ptr<rkp::ExceptionEncounteredSender> m_exceptionEncounteredSender;
...
void ExceptionEncounteredSenderTest::SetUp() {
    m_mockMessageSender = std::make_shared<rkpCommon::sdkInterfaces::test::MockMessageSender>();
    m_exceptionEncounteredSender = rkp::ExceptionEncounteredSender::create(m_mockMessageSender);
...
TEST_F(ExceptionEncounteredSenderTest, errorTypeUnexpectedInformationReceived) {
    ASSERT_TRUE(testExceptionEncounteredSucceeds(
        ...
        rkp::ExceptionErrorType::UNEXPECTED_INFORMATION_RECEIVED,
...
TEST_F(ExceptionEncounteredSenderTest, errorTypeUnexpectedOperation) {
    ASSERT_TRUE(testExceptionEncounteredSucceeds(
        UNPARSED_DIRECTIVE_JSON_STRING, rkp::ExceptionErrorType::UNSUPPORTED_OPERATION, "Operation not supported"));
}
/*
...
TEST_F(ExceptionEncounteredSenderTest, errorTypeInternalError) {
    ASSERT_TRUE(testExceptionEncounteredSucceeds(
        UNPARSED_DIRECTIVE_JSON_STRING, rkp::ExceptionErrorType::INTERNAL_ERROR, "An error occurred with the device"));
}
}  // namespace test
}  // namespace rkpCommon
}  // namespace localPlayerSDK