...
std::unique_ptr<rkpCommon::utils::HTTPContent> LibCurlHttpContentFetcher::getContent(FetchOptions fetchOption) {
    ...
    std::shared_ptr<rkpCommon::rkp::attachment::InProcessAttachment> stream = nullptr;
    switch (fetchOption) {
        ...
        case FetchOptions::ENTIRE_BODY:
            ...
            stream = std::make_shared<rkpCommon::rkp::attachment::InProcessAttachment>(m_url);
    ...
    return rkpCommon::utils::memory::make_unique<rkpCommon::utils::HTTPContent>(
        rkpCommon::utils::HTTPContent{std::move(httpStatusCodeFuture), std::move(contentTypeFuture), stream});
}
