...
int PlaylistParser::parsePlaylist(
    std::string url,
    std::shared_ptr<rkpCommon::utils::playlistParser::PlaylistParserObserverInterface> observer,
...
void PlaylistParser::doDepthFirstSearch(
    ...
    while (!urlsToParse.empty() && !m_shuttingDown) {
        ...
        auto httpContent = contentFetcher->getContent(
            rkpCommon::sdkInterfaces::HTTPContentFetcherInterface::FetchOptions::CONTENT_TYPE);
        if (!httpContent || !(*httpContent)) {
            ACSDK_ERROR(LX("getHTTPContent").d("reason", "badHTTPContentReceived"));
            observer->onPlaylistEntryParsed(
                id, urlAndInfo.url, rkpCommon::utils::playlistParser::PlaylistParseResult::ERROR, urlAndInfo.length);
            return;
        }
        ...
        if (contentType.find(M3U_CONTENT_TYPE) != std::string::npos) {
            ...
            if (!getContentFromPlaylistUrlIntoString(urlAndInfo.url, &playlistContent)) {
                ...
                observer->onPlaylistEntryParsed(
                    id,
                    urlAndInfo.url,
                    rkpCommon::utils::playlistParser::PlaylistParseResult::ERROR,
                    urlAndInfo.length);
                return;
            }
            ...
            if (std::find(
                ...
                observer->onPlaylistEntryParsed(
                    id,
                    urlAndInfo.url,
                    urlsToParse.empty() ? rkpCommon::utils::playlistParser::PlaylistParseResult::SUCCESS
                                        : rkpCommon::utils::playlistParser::PlaylistParseResult::STILL_ONGOING,
                    urlAndInfo.length);
                continue;
            }
