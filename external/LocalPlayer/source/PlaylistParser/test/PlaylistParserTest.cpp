...
using namespace rkpCommon::utils::playlistParser;
...
public:
    ...
    std::unique_ptr<rkpCommon::utils::HTTPContent> getContent(FetchOptions fetchOption) {
 ...
 */
class TestParserObserver : public rkpCommon::utils::playlistParser::PlaylistParserObserverInterface {
public:
...
TEST_F(PlaylistParserTest, testParsingPlaylist) {
    ...
    for (unsigned int i = 0; i < results.size(); ++i) {
        ...
        if (i == results.size() - 1) {
            ASSERT_EQ(results.at(i).parseResult, rkpCommon::utils::playlistParser::PlaylistParseResult::SUCCESS);
        } else {
            ASSERT_EQ(results.at(i).parseResult, rkpCommon::utils::playlistParser::PlaylistParseResult::STILL_ONGOING);
        }
    }
}
