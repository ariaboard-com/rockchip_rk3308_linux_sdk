...
static int run_cmd_to_shell_duplex(char *cmd, char *w_buf, char *r_buf, char *match_str)
{
    ...
    log_info("before  pclose start......\n");
    int t = pclose(fp);
    log_info("after  pclose run end......\n");
...
static int pcba_test_result_rw(PCBA_SINGLE_PARA *recv_paras, char *w_buf, char *r_buf, bool rw)
{
    ...
    if (rw) {
        ...
        fd = open(result_filename, O_RDWR);
        if (fd < 0) {
...
//  memcpy(start, "echo;", len);
    len = strlen(PCBA_TEST_PLATFORM_3308";");
    memcpy(start, PCBA_TEST_PLATFORM_3308";", len);
    start += len;
...
static int exit_pcba_test_mode(PCBA_SINGLE_PARA *recv_paras, char *test_flag)
{
    ...
    if (dir = opendir(TEST_RESULT_SAVE_PATH)) {
...
static int pcba_test_item_process(PCBA_SINGLE_PARA *recv_paras)
{
    ...
    log_info("pcba_test_item_process in 00000\n");
    ...
    if (ret) {
        ...
        log_info(" run_cmd_to_shell_duplex ret = 0 \n");
    }
    ...
    log_info("pcba_test_item_process out 11111\n");
